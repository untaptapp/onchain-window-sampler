...
- `pump_universe.py` + `.github/workflows/pump-universe.yml` — the **pump.fun launch firehose** (`pump_launches`, `schema_pump_launches.sql`) via the free PumpPortal websocket. This is the **population at risk** for the prediction study, and it exists because the REST-based `candidate_universe` **failed its coverage gate**: only 3.7–9.4% of tokens that went on to trend were in it beforehand, since GeckoTerminal's paged listings reach only the top ~200 by 24h volume and the ~200 newest pools — a bimodal sample missing the $10k–$1M band where trending happens. Recording every token at birth fixes that at the source: **84.3% of observed trending mints are pump.fun tokens** (88.6% of resolved DEXes are pumpswap/pump-fun), so every future trender in that share necessarily passes through this stream first. **Measured limits:** `subscribeNewToken` (~29/min ≈ 41k/day) and `subscribeMigration` are free; **`subscribeTokenTrade` is NOT** — it needs an API key funded with ≥0.02 SOL — so this supplies the population, birth-time stats and graduation events, but **not volume/CVD**. At ~7 MB/day retention is mandatory: `prune_pump_launches()` runs server-side on every flush (5 days for dead launches, 30 for graduated ones).
